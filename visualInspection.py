...
def putText(frame, background, text, x, y, w, h, colorFont):
    cv2.putText(background, text, (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 2)
    fx, fy, fw, fh = cv2.boundingRect(background[:, :, 2])
    cv2.rectangle(frame, (x, y), (x + w, y + h), colorFont, 2)
    cv2.rectangle(frame, (fx, fy), (fx + fw, fy + fh), colorFont, -1)
    cv2.rectangle(frame, (fx, fy), (fx + fw, fy + fh), colorFont, 3)
    cv2.putText(frame, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)


def imageBlur(image, left, top, w, h):
    topLeft = (left, top)
    bottomRight = (w, h)
    x, y = topLeft[0], topLeft[1]
    w, h = bottomRight[0] - topLeft[0], bottomRight[1] - topLeft[1]
    ROI = image[y:y+h, x:x+w]
    blur = cv2.GaussianBlur(ROI, (51, 51), 80)
